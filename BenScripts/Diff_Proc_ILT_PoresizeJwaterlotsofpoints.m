...
diffData = ('Sample7_2D81_n128_s512_16July2014.tnt');
t2Data = ('Sample7_n69_nE512_s128_21July2014.tnt');
saveData = 1; % 1if you want it to save ILT data, 0 if not
saveName = ('Sample7'); %the first part of the name it will be saving it as

diffusion = 0; % use 1 to used amp diff points in fit, 0 for summed
...
spec2 = real(spec2(1:nrExp*deltaStep,:));% just in case you have two many points or blank points
...
% deltaMin = Td(k) * 0.1; % these are for the longer overnight diffusion experiments
% deltaMax = Td(k) * 0.4;% these are for the longer overnight diffusion experiments
deltaMin = 7e-6;
deltaMax = 31e-6;
...
% sp2 = reshape(spplotec3(2,:),ap.td(2),ap.td(1));
...
guesses = [8000,.002,600,.001]; % for Jamestown water
...
for i = 1:deltaStep%size(sumData2,3)
    ...
    [tau(i).beta,tau(i).resid,tau(i).J] = nlinfit(echoAxis(2:end),sumDataPoint1(1,2:end,i),@t2bifit_simple,guesses,fitopts);
...
%     figure(i)
%     hold on
%     plot(echoAxis(2:end),tau(i).pred(2:end))
%     plot(echoAxis(2:end),sumDataPoint1(1,2:end,i),'-r')
end
...
fitVal = 6; % truncates points before this point for the slope fit and plot

[a,b] = polyfit(timeAxis(fitVal:end)',dEach(fitVal:end),1);
...
figure(3)
hold on
scatter((Td(fitVal:end).^0.5),dEach(fitVal:end))
...
guesses = [0.25,0.4832,0.054,0.3,0.02]; % for lsq fitting lime
lowerBounds = [0,0,0,0,0];
upperBounds = [1,inf,inf,inf,inf];
fitopts = statset('MaxIter',500,'TolX',1e-14,'UseParallel',true,'Display','off');

[lsqfit.beta,lsqfit.resid,lsqfit.J] =lsqcurvefit(@bifitH20Scale,guesses,echoAxis,dataNorm,lowerBounds,upperBounds);
...
dataDif = dataDif./max(dataDif); % renormalizes data after the removal of water is done

[fit.beta,fit.resid,fit.J] = nlinfit(echoAxis(1:length(dataDif)),dataDif,@t2bifit_simple,guesses,fitopts);
...
lowLim = 0.01e-3; %min(echoVector)/10000; %
