...
diffData = ('Armory_2D81_n128_s512_27June2014.tnt');
t2Data = ('Jamestown_n32_nE512_s128_1July2014.tnt');
saveData = 1; % 1if you want it to save ILT data, 0 if not
saveName = ('Armory'); %the first part of the name it will be saving it as
...
nrExp = 9;
nrEchoes = 128;
wantedNrEchoes = 64%nrEchoes;% this is used for trimming the echoes if you want less to get better data and fits
nr2DPnts = ap.td(2);
nr3DPnts = ap.td(3);
blankPnts = 5; %number of "zero" points at ends of acq windows
Td = linspace(200e-6,3400e-6,nrExp); %Td time in s for each experiment
nrScans = 512;
...
dEach = [dEach;diff(l).amp]; %converts from structure to a matrix
end

[a,b] = polyfit(timeAxis(2:end)',dEach(2:end),1);
sv = a(1)/(-a(2)^1.5*(4/9/pi^0.5));

fitSlope= num2str(real(a(1)));% I got clever with my figures ;)
fitInt = num2str(real(a(2)));% oh yes, CLEVER.
fitLegend = strcat('Y = ',fitSlope,'x + ',fitInt);% where the magic happens
figure(5)
hold on
scatter((Td(2:end).^0.5),dEach(2:end))
legend(fitLegend) %BAM it displays
xlabel ('time^2')
ylabel('diff value m/s^2')
...
% exponential fitting
guesses = [0.4832,0.047,0.3,0.02,0.15,0.004];% for armory triexpfit
% guesses = [0.8,0.027,0.2,0.005];% for lime mortar
% guesses = [0.9,0.003,0.14,0.00093];% for hydraulic mortar
fitopts = statset('MaxIter',500,'TolX',1e-14,'UseParallel',true,'Display','off');

[fit.beta,fit.resid,fit.J] = nlinfit(echoAxis(1:length(dataNorm)),dataNorm,@t2trifit,guesses,fitopts);
[fit.pred] = t2trifit(fit.beta,echoAxis(1:length(dataNorm)));
ci = nlparci(fit.beta,fit.resid,'jacobian',fit.J);
[fit.cil] = t2trifit(ci(:,1),echoAxis(1:length(dataNorm)));
[fit.ciu] = t2trifit(ci(:,2),echoAxis(1:length(dataNorm)));

% close all
figure(1)
hold on
plot(echoAxis(1:length(dataNorm)),dataNorm)
plot(echoAxis(1:length(dataNorm)),fit.pred)
plot(echoAxis(1:length(dataNorm)),fit.ciu,'-r')
plot(echoAxis(1:length(dataNorm)),fit.cil,'-k')
legend('data','pred','upperci','lowerci')

%creating box of t2 dist as errorbars
t21A = fit.beta(1)/(fit.beta(1)+fit.beta(3)+fit.beta(5));%relative abundance of the first t2 based on amp
...
t22A = fit.beta(3)/(fit.beta(1)+fit.beta(3)+fit.beta(5));% relative abun of the 2nd t2 based on amp
t22 = linspace(ci(4,1),ci(4,2),100);% creates a scale of t2 values based on ci
t22AV = ones(length(t22),1).*t22A;% creates the vector that's the same size as t21

t23A = fit.beta(5)/(fit.beta(1)+fit.beta(3)+fit.beta(5));%relative abundance of the first t2 based on amp
t23 = linspace(ci(6,1),ci(6,2),100);% creates a scale of t2 values, based on ci
t23AV = ones(length(t23),1).*t23A; % creates the vector that's the same size as t21
...
normILT = normILT * t21A; % multiplied by the relative t21 abundance to give values of the curves
...
plot(t23,t23AV,'-g','LineWidth',2)
stem(fit.beta(2),t21A,'-m')
stem(fit.beta(4),t22A,'-c')
stem(fit.beta(6),t23A,'-g')
xlabel('t2 times [s]')
ylabel('relative percent abundance')
legend('Integrated','Distribution','first T2 value from fit','2nd T2 Value from fit','3rd T2 value from fit')
...
plot(xScalar*t23,t23AV,'-g','LineWidth',2)
stem(xScalar*fit.beta(2),t21A,'-m')
stem(xScalar*fit.beta(4),t22A,'-c')
stem(xScalar*fit.beta(6),t23A,'-g')
...
save(strcat(saveName,'t23Ax.txt'),'t23Ax','-ascii'); % the produced t2 value from the fit
save(strcat(saveName,'t23x.txt'),'t23x','-ascii'); % the t2 points from the ci's
save(strcat(saveName,'t23A.txt'),'t23A','-ascii'); % the  T2 points weighted amp.

elseif saveData == 0;
    print('notsaved data = lost data')
else
end
