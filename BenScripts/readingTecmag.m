...
dirTM = ('C:\Users\bmfortman\Documents\Data\Tecmagtest');
cd(dirTM)

%% reading Data from a tecmag file
[ap,s1,s2] = readTecmag('15Cglycerol.tnt'); %reads the data from the file all are complex points
...
s3 = reshape(s2Ac,32,256); % # acq pts, # echoes, # 2dexperiments
% surf(abs(s3(:,:,3)));
s2sum = sum(s3,1);
% s2sum = reshape(s2sum,256);%summed data, #Echoes, #2d experiments gives you summed echoes for all echoes in the train
s2sumAbs = abs(s2sum);
% surf(s2sum)
% surf(abs(s2sum))
echoAxis = linspace(150,256*150,256); %gives your time axis #echotime, #number echoes*Echotime, numberEchoes(assuming dwell time of 1u)
