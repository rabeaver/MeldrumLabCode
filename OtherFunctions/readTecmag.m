function [ap,spec,spec2] = readTecmag(pathname)
% -----------------------------------------------------------------
...
elseif td2 > 1,
    ...
    for k=1:td2
        ...
        if cnt == td1*2
            spec2(k,:)  = a(1:2:cnt) - 1j*a(2:2:cnt);
%         else
%             k
%             break;
        end
...
fseek(fileid,paLen+tsLen+12+daLen+8,'bof');
ts2Len      = fread(fileid,1,'int32'); %length of Tecmag2 struct

% --- start of PSEQ struct
fseek(fileid,paLen+tsLen+12+daLen+12+ts2Len+16,'bof');
fnlength     = fread(fileid,1,'int32'); %length of filename
pSeq         = fread(fileid,fnlength,'*char'); 
nRow      = fread(fileid,1,'int32');
nCol      = fread(fileid,1,'int32');
fseek(fileid,paLen+tsLen+12+daLen+12+ts2Len+16+12+fnlength+24,'bof');
dsLen        = fread(fileid,1,'int32');
ds           = fread(fileid,dsLen,'*char');
lsLen        = fread(fileid,1,'int32');
ls           = fread(fileid,lsLen,'*char');
% -- end of PSEQ struct
...
% ap.ds           = deblank(ds');
% ap.ls           = deblank(ls');
ap.pulprog     = deblank(pSeq');

% line = fgetl(fileid);
% while (~strncmp(line,'nrEchoes',5))&&(~feof(fileid))
%     line = fgetl(fileid);
% %     if strncmp(line,'nrEchoes',8)
% %         aq.sqParPos = ftell(fileid);
% %     end
% end
% ap.line = line;
% k = strfind(line,'nrEchoes');
% ap.nrEchoes = str2double(line(k+16:k+19));
% % ap.sqPos = ftell(fileid);

% fseek(fileid,ap.sqPos,'bof');
% ap.nrParams = fread(fileid,1,'int32');

% 
% 
% %    sqPar=fgetl(fileid);
% %    while (~strncmp(sqPar,'nrEchoes',8))&&(~feof(fileid))
% %        sqPar=fgetl(fileid);
% %        ap.sqParPos=ftell(fileid);
% %    end
% 
% fseek(fileid,ap.sqParPos,'bof')
% ap.nrPars   =   fread(fileid,1,'int32');
% --- sequence parameters
