...
%guesses for T2-D
T2guess = [0.008]; %s
Dguess = [3.0e-9];         % diffusion guess (m^2/s)
...
nTrials = 5;        % times to iterate noise
...
[spec] = T2Dsim(nEchoes+2, tEcho, T2sim, G, DELTA, Dsim, deltaMin, deltaMax, nDPnts, SNR);
[specguess] = T2Dsim(nEchoes+2, tEcho, T2guess, G, DELTA, Dguess, deltaMin, deltaMax, nDPnts, SNR);
...
spec = reshape([specRe, specIm]', 2*nDPnts*(nEchoes+2), 1);

specReGuess = real(specguess);
specImGuess = specIm;
specguess = reshape([specReGuess, specImGuess]', 2*nDPnts*(nEchoes+2), 1);
...
opts.priorCov = diag([10^2*ones(length(T2guess),1); 10000^2*ones(length(T2guess),1); 1000^2*ones(length(Dguess),1); 100^2*ones(2,1)]);
opts.tau = tEcho;
opts.sigma2 = (1./SNR)^2;
opts.D = Dguess;
opts.G = G;
...
for iTrial=1:nTrials
    ...
    [T2Hat(:,iTrial), wHat(:,iTrial), DHat(:,iTrial),FHat(:,iTrial),PHat(:,iTrial)] = bayesian_estimate(y,opts);
