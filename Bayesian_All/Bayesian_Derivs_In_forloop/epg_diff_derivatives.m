...
% Date: June 2012
%
function [ Jr, Jd, Ja ] = epg_diff_derivatives( n, tau, R1, R2vec, alpha, D, G, delta, DELTA)
...
T0d = [-0.5*sin(alpha), 0.5*sin(alpha), cos(alpha); ...
    0.5*cos(alpha), -0.5*sin(alpha), -cos(alpha); ...       % Had sin(alpha) at 2,1
...
T1d = [-0.25*sin(alpha), 0.25*sin(alpha), 0.5*cos(alpha); ...
    0.25*cos(alpha), -0.25*sin(alpha), -0.5*cos(alpha); ...
...
%    Rd = blkdiag(-tau*exp(-tau*R2),RArray{:});
    ...
    DArrayDELTAd{:} = sparse(diag([bTDELTA*exp(-D*bTDELTA),bTDELTA*exp(-D*bTDELTA),bLDELTA*exp(-D*bLDELTA)]));
...
%    DArrayDELTA{:}  = sparse(diag([1,1,1]));
    ...
    %DArrayd = cell(1,n);
    %for i = 1:n
        bT = ((3-1)*k+k/2)^2*tau+k^2/12*tau; 
        bL = ((3-1)*k)^2*tau;
        DArray = diag([exp(-D*bT),exp(-D*bT),exp(-D*bL)]);
        DArrayd = diag([bT*exp(-D*bT),bT*exp(-D*bT),bL*exp(-D*bL)]);
    %end
    Dhold = cell(1, n);
    Dhold(:) = {sparse(DArray)};
    D2 = blkdiag(1,DArrayDELTA{:},DArraydelta{:},Dhold{:});
    
    Ddhold = cell(1, n);
    Ddhold(:) = {sparse(DArrayd)};
    D2d = blkdiag(0,DArrayDELTAd{:}, DArraydeltad{:}, Ddhold{:});
