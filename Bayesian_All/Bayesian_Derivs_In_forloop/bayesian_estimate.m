...
nEchoes = opts.nEchoes + 2;                 % Pass nEchoes as a variable of opts
N = 2*(nEchoes)*nDPnts;            % Number of measurements (real & imag)
...
%     p_prev = mu_prev(end);                          % Phase
      ...
      Jg = zeros(nEchoes, nDPnts);
      Jd = zeros(nEchoes, nDPnts);
      Ja = zeros(nEchoes, nDPnts);
%     decayIn = exp(-gamma.^2 .* G.^2 .* deltaVec.^2 .* (d_prev) .* (DELTA + (2*deltaVec./3)));
    % Get EPG signal (real and imaginary components)
    for l = 1:nDPnts
        Hx(:,l) = epg_diff(nEchoes,tau,1,g_prev,a_prev, d_prev, G, deltaVec(l), DELTA) * exp(1j*p_prev);
        [Jg(:,l), Jd(:,l), Ja(:,l)] = epg_diff_derivatives(nEchoes,tau,1,g_prev,a_prev, d_prev, G, deltaVec(l), DELTA);
    end
...
%     plot(diff(J(:,5)));
%     
    Psi = (J*Psi_prev*J' + (sigma2*eye(N)))/gammas(j);
    % ScaledSigma = gammas(j)/sigma2*eye(N);
    Kf = Psi_prev*J'/Psi; %(J*Psi_prev*J'+ ScaledSigma);
