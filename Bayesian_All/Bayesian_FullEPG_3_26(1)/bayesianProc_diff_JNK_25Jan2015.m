...
%guesses for T2-D
T2guess = [0.005]; %s
Dguess = [2.2e-9];         % diffusion guess (m^2/s)
...
nTrials = 3;        % times to iterate noise
...
opts.priorCov = diag([0.01^2*ones(length(T2guess),1); 10000^2*ones(length(T2guess),1); 1^2*ones(length(Dguess),1); 0.05^2*ones(2,1)]);
opts.tau = tEcho;
opts.sigma2 = (1./SNR)^2;
opts.D = Dguess;
opts.G = G;
